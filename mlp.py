...
class MLP:
        ...
        for l in range(0, self.L + 1):
            ...
            for i in range(0, self.d[l - 1] + 1):
                for j in range(0, self.d[l] + 1):
                    self.W[l, i, j] = 0.0 if j == 0 else random.random() * 2.0 - 1.0
    ...
    def predict(self, sample_inputs: np.ndarray, is_classification: bool = True) -> np.ndarray:
        ...
        raw_output = self.X[self.L, 1:self.d[self.L] + 1]
        return softmax(raw_output) if is_classification else raw_output
